...
- [~] The four worked examples in strategy-matrix §10 are encoded as regression tests and pass. *(Partial — the two fully-directional examples this phase covers end-to-end are encoded and green: **Example A** (AAPL classic high-IV bullish credit spread → BULL_PUT_CREDIT_SPREAD, Standard-band 0.20Δ short put, credit economics, credit↔HIGH match score, §7 sizing $50k×3%/$320 → 4 contracts) and **Example D** (NVDA low-IV high-conviction long call → LONG_CALL unlock, 0.65Δ, premium=max-loss, long↔LOW match score). Both drive matrix → selector over the BS-priced synthetic chain and pin the deterministic claims the engine owns (mapping, delta targets, economics identities, regime-fit factor, sizing rule), not the doc's illustrative real-ticker dollar values. Tests: `DirectionalStrategySelectorTest.section10ExampleA_classicHighIvBullishCreditSpread` + `section10ExampleD_lowIvHighConvictionLongCall` (35 in class; backend suite 213 green). **Example C** (MSFT neutral high-IV Covered Call) is now encoded with Phase 6 AC1's income-overlay machinery — `IncomeOverlaySelectorTest.section10ExampleC_neutralHighIvCoveredCall`. **Still deferred:** Example B (`WEAK_SIGNAL` NEUTRAL abstain) depends on the Phase 6 AC3 neutral/abstain overrides, which don't exist yet — this line flips to `[x]` when that lands.)*
