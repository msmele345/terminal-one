...
- [ ] Tests: provider adapter (against recorded fixtures), cache TTL behavior, P&L math. *(Cache-TTL (`CachingMarketDataClientTest`) + P&L math (`PositionPnlTest`, `PortfolioSummaryServiceTest`) done; the provider-adapter fixture tests exist but are gated on golden fixtures not yet captured — they `assumeTrue`-skip until `spikes/capture-marketdata-fixture.mjs` records them.)*
